...
use super::theme;
use crate::{app::App, data::socials::BUILDING};
...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Min(6),                                      // bio + building
            Constraint::Length(7),                                   // quote
        ])
        .split(area);

    render_wordmark(f, chunks[0]);
    render_tagline(f, chunks[1]);
    render_bio_and_building(f, app, chunks[3]);
    render_quote(f, app, chunks[4]);
}
...
fn render_tagline(f: &mut Frame, area: Rect) {
    let line = Line::from(vec![
        ...
        Span::styled("  ·  ", theme::secondary()),
        Span::styled("economic justice advocate", theme::body()),
        Span::styled("  ·  ", theme::secondary()),
